...
face_exp_model = model_from_json(open("C:/Users/drsan/face mask/code/facial_expression_model_structure.json","r").read())
face_exp_model.load_weights('C:/Users/drsan/face mask/code/facial_expression_model_weights.h5')
#declare the emotions label
emotions_label = ('angry', 'disgust', 'fear', 'happy', 'sad', 'surprise', 'neutral')


#initialize array var to hold all face locations in frames
all_face_locations=[]
#loop through every frame in video
while True:
    #get current frame of video as image
    ret,current_frame = webcam_video_stream.read()
    #rezie current frame
    current_frame_small=cv2.resize(current_frame,(0,0),fx=0.25, fy=0.25)
    #detect all faces in image
    all_face_locations=face_recognition.face_locations(current_frame_small,number_of_times_to_upsample=3,model="hog")
    
    
    for index,current_face_location in enumerate(all_face_locations):
        #splitting tuple to get 4 position values
